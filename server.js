...
app.post('/api/confirmar', async (req, res) => {
  ...
  const presente = (req.body.presente || '').trim();
  ...
  await notificarWhatsApp(nome, presente);

  res.json({ ok: true });
});

async function notificarWhatsApp(nome, presente) {
  ...
  let texto = `🎉 Nova confirmação de presença!\nNome: ${nome}`;
  if (presente) texto += `\nPresente: ${presente}`;
